...
def mfcc_from_file(
    audio_path, n_mfcc=config["n_mfcc"], sample_interval=config["sample_interval"], window_len=config["window_len"]
):
    # load audio to time serial wave, a 2D array
    audio, sr = librosa.load(audio_path, sr=None)
    mfccs = librosa.feature.mfcc(
        y=audio, sr=sr, n_mfcc=n_mfcc, hop_length=int(sample_interval * sr), n_fft=int(window_len * sr)
    )
...
def split_input_target(mfccs):
    # assert mfccs.shape[1] == parameters.shape[1], 'Sequence length in mfcc and parameter is different'
    # assert phonemes.shape[1] == parameters.shape[1], 'Sequence length in phoneme and parameter is different'
    seq_len = mfccs.shape[1]
    inputs = mfccs
    # target: parameter at a time, input: sliding window (past 80, future 20)
    input_list = []
